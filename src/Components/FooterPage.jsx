...
export default function FooterPage({onClick, onPrev, onNext, pageNum1, pageNum2}) {
  return (
    <>
      <div className="fixed bottom-0 right-0 w-full h-16 bg-[#c4122f] text-white flex items-center justify-between">
        <div className="pl-4 flex items-center justify-between gap-4 select-none">
            ...
            <div className="w-10 h-10 bg-white text-[#c4122f] flex items-center justify-center rounded-full cursor-pointer" onClick={onPrev}>
                <FiChevronLeft className="text-2xl" />
            </div>
        </div>
        <div className="flex flex-col items-center justify-center gap-10 cursor-pointer" onClick={onClick}>
          <img
            src={moon}
            alt="Moon"
